...
int main(){
	...
	for(int l = 1; l <= n; ++l){
		for(int r = l; r <= n; ++r){
			if((sum[r] ^ sum[l - 1]) == k){x[cnt].l = l; x[cnt++].r = r;}
		}
	}
	sort(x, x + cnt, 
		[](Range q, Range p){return (p.r == q.r ? p.l > q.l : p.r > q.r);}
	);
//	cout << cnt << endl;
//	for(int i = 0; i < cnt; ++i) cout << x[i].l << ' ' << x[i].r << endl;
	int last = -1, ans = 0;
	for(int i = 0; i < cnt; ++i){
		if(x[i].l > last) ++ans, last = x[i].r;
	}
	cout << ans;
	return 0;
}
