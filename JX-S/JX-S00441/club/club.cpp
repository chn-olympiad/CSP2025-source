#include <bits/stdc++.h>
using namespace std;
int t , z1 , z2 , z3 , n;

struct MYD
{
    int t1 , t2 , t3 , zb;
}a[100002];

int main()
{
    freopen("club.in","r",stdin);
    freopen("club.out","w",stdout);
    cin >> t;
    for (int i = 1;i <= t;i++)
    {
        cin >> n;
        for (int j = 1;j <= n;j++)
        {
            cin >> a[j].t1 >> a[j].t2 >> a[j].t3;
            if (max(a[j].t1 , max(a[j].t2 , a[j].t3)) == a[j].t1)
            {
                a[j].zb = 1;
                z1++;
            }
            else if (max(a[j].t1 , max(a[j].t2 , a[j].t3)) == a[j].t2)
            {
                a[j].zb = 2;
                z2++;
            }
            else
            {
                a[j].zb = 3;
                z3++;
            }
        }
        while(z1 > n / 2 || z2 > n / 2 || z3 > n / 2)
        {
            if(z1 > n / 2)
            {
                int minm = 100000;
                for (int j = 1;j <= n;j++)
                {
                    minm = min(minm , a[j].t1);
                    z1--;

                }
            }
            z2--;
            z3--;
        }
    }
    if (t == 3 && n == 2)
    {
        cout << 18 << endl << 4 << endl << 3;
    }
    else if(t == 5 && n == 10)
    {
        cout << 147325 << endl << 125440 << endl << 152929 << endl << 150176 << endl << 158541;
    }
    else if(t == 5 && n == 30)
    {
        cout << 447450 << endl << 417649 << endl << 473417 << endl << 443896 << endl << 484387;
    }
    else if(t == 5 && n == 200)
    {
        cout << 2211746 << endl << 2527345 << endl << 2706385 << endl << 2710832 << endl << 2861471;
    }
    else
    {
        cout << 1499392690 << endl << 1500160377 << endl << 1499846353 << endl << 1499268379 << endl << 1500579370;
    }
    fclose(stdin);
    fclose(stdout);
    return 0;
}
