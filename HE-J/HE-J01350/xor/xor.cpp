...
一
.
次。例如，若 s 为 1a01b，则小 R 可以同时选择第 1,3,4 个字符，分别为
...
相
.
同。所有 n × m 名考生将按照 CSP-J 2025 第一轮的
成绩，
.
由
.
...
到
.
低分别为 s 1 > s 2 > ··· > s n×m ，则成绩为 s 1 的考生的座位为第 1
.
列
第 1
.
行，成绩为 s 2 的考生的座位为第 1
.
列第 2
.
行，……，成绩为 s n 的考生的座位为
...
【输入格式】
从文件 seat.in 中读入数据。
输入的第一行包含两个正整数 n,m，分别表示小 R 所在的考场座位的
.
行
.
数与
...
1 1 2
【样例 1 解释】
按照成绩从高到低的顺序，成绩为 100 的考生的座位为第 1
.
列第 1
.
行，成绩为 99
...
示二进制按位异或。
小 X 给了小 R 一个非负整数 k。小 X 希望小 R 选择序列中尽可能多的
.
不
.
相
...
不
.
同，即存在 1 ≤ i ≤ n，使得其中一种方案选择了第 i 根小木棍，但另一种方
