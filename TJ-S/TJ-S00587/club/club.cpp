...
bool cmp1(ini x,ini y){
	if(x.f_ini == 0 && y.f_ini == 0){
		if(x.i1_ini != y.i1_ini) return x.i1_ini>y.i1_ini;
		else if(x.i_ini == 1 && y.i_ini != 1){
			return 1;
		}
		else if(y.i_ini == 1 && x.i_ini != 1){
			return 0;
		}
		else{
			return (x.i2_ini+x.i3_ini)<(y.i2_ini+y.i3_ini);
		}
	}
	return x.f_ini<y.f_ini;
}

bool cmp2(ini x,ini y){
	if(x.f_ini == 0 && y.f_ini == 0){
		if(x.i2_ini != y.i2_ini) return x.i2_ini>y.i2_ini;
		else if(x.i_ini == 2 && y.i_ini != 2){
			return 1;
		}
		else if(y.i_ini == 2 && x.i_ini != 2){
			return 0;
		}
		else{
			return (x.i1_ini+x.i3_ini)<(y.i1_ini+y.i3_ini);
		}
	}
	return x.f_ini<y.f_ini;
}

bool cmp3(ini x,ini y){
	if(x.f_ini == 0 && y.f_ini == 0){
		if(x.i3_ini != y.i3_ini) return x.i3_ini>y.i3_ini;
		else if(x.i_ini == 3 && y.i_ini != 3){
			return 1;
		}
		else if(y.i_ini == 3 && x.i_ini != 3){
			return 0;
		}
		else{
			return (x.i1_ini+x.i2_ini)<(y.i1_ini+y.i2_ini);
		}
	}
	return x.f_ini<y.f_ini;
}
...
int main(){
	...
	while(t--){
		cin>>n;
		for(int i=0;i<n;i++){
			cin>>a[i].i1_ini>>a[i].i2_ini>>a[i].i3_ini;
			a[i].f_ini = 0;
			if(a[i].i1_ini > a[i].i2_ini && a[i].i1_ini > a[i].i3_ini) a[i].i_ini = 1;
			if(a[i].i2_ini > a[i].i1_ini && a[i].i2_ini > a[i].i3_ini) a[i].i_ini = 2;
			if(a[i].i3_ini > a[i].i2_ini && a[i].i3_ini > a[i].i1_ini) a[i].i_ini = 3;
		}
		long long x = n/2;
		long long ans = 0;
...
∑ n
i=1 a i,d i 。
小 L 不希望某一个部门的新成员数量过多。具体地，他要求在分配方案中，
.
不
.
存
...
特殊性质 C：对于所有 1 ≤ i ≤ n，1 ≤ j ≤ 3，a i,j 均在 [0,2×10 4 ] 中
.
独
.
立
...
座城市间建造一条道路的费用为 a j,i 。C 国可以在这 k 个乡镇中选择
.
任
.
意
...
? 对于所有 1 ≤ i ≤ m，均有 1 ≤ u i ,v i ≤ n，u i ?= v i 且 0 ≤ w i ≤ 10 9 ；
...
=
∑ q
j=1 |t j,1 | + |t j,2 |。对于所有测试数据，保证：
? 1 ≤ n,q ≤ 2 × 10 5 ；
? 2 ≤ L 1 ,L 2 ≤ 5 × 10 6 ；
? 对于所有 1 ≤ i ≤ n，s i,1 ,s i,2 均仅包含小写英文字母，且 |s i,1 | = |s i,2 |；
? 对于所有 1 ≤ j ≤ q，t j,1 ,t j,2 均仅包含小写英文字母，且 t j,1 ?= t j,2 。
测试点编号 n,q ≤ L 1 ,L 2 ≤ 特殊性质
1,2 10 2 200
无
3 ～ 5
10 3
...
别
.
的，当且仅当字符串 s 仅包含字符 a 和 b，且
字符 b 在 s 中出现
.
恰
.
好一次。对于所有 1 ≤ i ≤ n，s i,1 ,s i,2 均为特别的，且对于所有
