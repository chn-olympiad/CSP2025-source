...
int n, a[N], res;
ll bag[N], sum[N<<2], ans;
void add(int idx,int l,int r,int x,ll val) {
	if (l==r) return sum[idx]=(sum[idx]+val)%M, void();
	int mid=(l+r)>>1;
	if (x<=mid) add(idx<<1,l,mid,x,val);
	else add(idx<<1|1,mid+1,r,x,val);
...
ll get(int idx,int l,int r,int ql,int qr) {
	...
	int mid=(l+r)>>1;
	ll c=0;
	if (ql<=mid) c=(c+get(idx<<1,l,mid,ql,qr))%M;
	if (qr>mid) c=(c+get(idx<<1|1,mid+1,r,ql,qr))%M;
	return c;
}
signed main(void) {
	...
	sort(a+1,a+n+1);
	for (int i=1;i<=n;i++) {
		if (i>2) ans=(ans+get(1,1,res,a[i]+1,res))%M;
		for (int j=res;j>=a[i];j--) {
			bag[j]=(bag[j]+bag[j-a[i]])%M;
			if (bag[j-a[i]]>0) add(1,1,res,j,bag[j-a[i]]);
		}
	}
	cout << ans << '\n';
	Blackdream1853 code;
...
----4
18
1 104 3 2 7 92 30 1 29 2 10 38 29 37 402 8 2 82
----109754
*/
