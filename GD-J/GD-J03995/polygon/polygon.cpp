...
//		cout<<"%%%%%%%%%%%"<<endl;
		...
		printf("%lld",(long long)(ans-1-n-n*(n-1)/2+2*mod)%mod);
//		cout<<(ans-1-n-n*(n-1)/2+2*mod)%mod;
	}else{
		sort(a+1,a+n+1);
		long long x;
		for(int i=1;i<=n;i++){
		...
		for(int i=1;i<=n;i++){
			j=1;
			for(int k=i-1;k>=2;k--){
				while(j+k-1<i&&sum[j+k-1]-sum[j-1]<=a[i]){
					j++;
				}
				if(j+k-1<i){
					ans+=c[i-1][k];
