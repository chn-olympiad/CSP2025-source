...
//	freopen("1.in","r",stdin);
	...
	{
//		cout<<i<<":: ";
//		for(int j=a[i]+1ll;j<=500;j++)cout<<f[j]<<" ";
//		cout<<"\n";
		for(int j=a[i]+1ll;j<=5001ll;j++)ans+=f[j]%mod,ans%=mod;//,cout<<f[j]<<" ";
//		cout<<"\n";
		for(int j=5001ll;j>=0;j--)f[min(j+a[i],5001ll)]+=f[j]%mod,f[min(a[i]+j,5001ll)]%=mod;
		ans%=mod;
	}
	printf("%lld\n",ans);
//	for(int i=1;i<=n;i++)
...
//			{
//				int mid=(l+r)/2;
//				if(sum[j]-sum[mid-1]>a[i])l=mid+1,w=mid;
//				else r=mid-1;
//			}
//			cout<<w<<" "<<j<<" "<<a[i]<<endl;
//			if(w!=-1)
//			{
//				ans+=ksm(2ll,i-(j-w+1)-1);
//				ans%=mod;
//			}
//		}
//	}
