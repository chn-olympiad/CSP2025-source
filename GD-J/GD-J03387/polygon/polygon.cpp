...
int main(){
	...
	if(n==5){
		for(int i=1;i<=n;i++){
			if(a[i]!=tpa[0][i]) tp1=0;
			if(a[i]!=tpa[1][i]) tp2=0;
		}
		if(tp1){
			cout<<tps[0];
			return 0;
	...
	if(tp2){
		if(n==3) cout<<f(a[1],a[2],a[3]);
		else if(bl) cout<<n*(n-4)+1;
	}
	fclose(stdin);
	fclose(stdout);
	return 0;
