...
int f[110][110][110], cnt[N], w[N], fac[N], inv[N];
char s[N];
...
signed main()
{
	...
	for(i=0;i<n;i++) for(j=0;j<=i;j++) for(k=0;k<=n;k++) if(f[i][j][k])
	{
		if(s[i+1]=='1')
		{
			...
			for(x=0;x<=i-(w[j]-k)&&x<=cnt[j+1]&&k+cnt[j+1]-x-1>=0;x++) Add(f[i+1][j+1][k+cnt[j+1]-x-1],f[i][j][k]*k%mod*A(i-(w[j]-k),x)%mod*C(cnt[j+1],x)%mod);
		}
		else
		{
			for(x=0;x<=i-(w[j]-k)&&x<=cnt[j+1]&&k+cnt[j+1]-x-1>=0;x++) Add(f[i+1][j+1][k+cnt[j+1]-x-1],f[i][j][k]*k%mod*A(i-(w[j]-k),x)%mod*C(cnt[j+1],x)%mod);
