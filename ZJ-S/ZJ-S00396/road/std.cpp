...
const int N = 3e4 + 5, M = 2e6 + 5, K = 10 + 5, T = (1 << 10) + 5;
...
signed main() {
	...
	for (int i = 1; i < (1 << k); ++i) {
		int t = __lg(i & (-i)) + 1;
		int lst = i ^ (i & (-i));
		int tot = __builtin_popcount(i) + n;
		val[i] = val[lst] + a[t];
		for (int j = 1, p1 = 1, p2 = 1; j <= len[lst] + n; ++j) {
			if (p2 > n || p1 < lst[len] && g[lst][p1].w < c[t][p2].w) {
				...
				cur[j] = node(c[t][p2].to, tot, c[t][p2].w);
				++p2;
			}
		}
		cout << "=====\n";
		// cerr << i << ' ' << calc(cur, g[i], len[i], tot, len[lst] + n) << '\n';
		ans = min(ans, val[i] + calc(cur, g[i], len[i], tot, len[lst] + n));
	}
	cout << ans << '\n';
