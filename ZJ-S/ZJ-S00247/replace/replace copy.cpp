...
pii cur[maxm];
int cur_len;
void dfs2(int x,int ll,int rr,int dep){
	if(ll>rr) return;
	if(S[x].size()){
		for(int i=ll;i<=rr;i++) h[i]=0;
		for(int i:S[x]){
			int l=pl[i],r=pr[i];
			l=lower_bound(cur+ll,cur+rr+1,mkp(l,-1))-cur;
			r=upper_bound(cur+ll,cur+rr+1,mkp(r,1000000000))-cur;
			if(l<r) h[l]++,h[r]--;
		}
		for(int i=ll;i<=rr;i++) {
			if(i!=ll) h[i]+=h[i-1];
			ans[cur[i].second]+=h[i];
		}
	}
	int ppl[26],ppr[26];
	memset(ppl,63,sizeof ppl);
	for(int i=ll;i<=rr;i++){
		if(dep>=c[cur[i].second].size()) continue;
		ppl[c[cur[i].second][dep]-'a']=max(ppl[c[cur[i].second][dep]-'a'],i);
		ppr[c[cur[i].second][dep]-'a']=min(ppr[c[cur[i].second][dep]-'a'],i);
	}
	for(int i=0;i<26;i++) if(trie[x][i]) dfs2(trie[x][i],ppl[i],ppr[i],dep+1);
}
void solve(int p){
	// puts("----");
	...
	for(int i:S2[p]){
		...
		val[i]=dfn[x];
		cur[cur_len++]=mkp(val[i],i);
		// printf("S2: %d %d\n",i,val[i]);
	}
	sort(cur,cur+cur_len);
	for(int i:S1[p]){
		int x=1;
