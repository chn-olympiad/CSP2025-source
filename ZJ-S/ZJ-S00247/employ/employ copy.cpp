...
int main(){
    ...
    for(int i=0;i<N;i++){
        ...
        if(F[i][j][k]){
            int val=F[i][j][k];
            if(s[i+1]=='0'){
                for(int l=min(A[j+1],k);l>=0;l--) F[i+1][j+1][k-l]=(F[i+1][j+1][k-l]+1ll*val*C[k][l]%Mod*C[A[j+1]][l]%Mod*fac[l])%Mod;
            }
            if(s[i+1]=='1'&&Pre[j]-(sum-k)>0){
                int v=1ll*val*(Pre[j]-(sum-k))%Mod;
                for(int l=min(A[j+1],k);l>=0;l--) F[i+1][j+1][k-l]=(F[i+1][j+1][k-l]+1ll*v*C[k][l]%Mod*C[A[j+1]][l]%Mod*fac[l])%Mod;
            }
            if(s[i+1]=='1'){
