...
struct nde {
	...
	inline void init(int _id) {
		...
		u = s.substr(0, l), v = s.substr(r + 1, siz(s) - (r + 1));
		d.clear();
		for (int i = l; i <= r; ++i) d += s[i], d += t[i];
		return;
	}
} a[N];
int ans[N];
...
void NamespaceMain() {
	...
	sort(a + 1, a + n + qn + 1, [&](const nde &A, const nde &B) -> bool { return A.d < B.d; });
	...
	for (int r; l <= n + qn; l = r) {
		...
		// cerr bug(l) bug(r) << endl;
		// for (int i = l; i < r; ++i) cerr bug(a[i].s) bug(a[i].t) bug(a[i].d) << endl;
...
int main() {
	...
	ignore = freopen("replace.out", "w", stdout);
