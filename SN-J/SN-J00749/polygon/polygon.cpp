...
bool ch1() {
	for(int i=1; i<=n; i++) {
		if(a[i]!=t1[i]) return 0;
	}
	return 1;
}

//void dfs(int i,int y,int mx,int sum) { //µ±Ç°i£¬²îy
//	if(i>=n) return;
//	if(y==0){
//		if(sum>mx*2){
//			ans++;
...
//		return;
//	}
//	dfs(i+1,y-1,max(mx,a[i]),sum+a[i]);
//	dfs(i+1,y,mx,sum);
//}
