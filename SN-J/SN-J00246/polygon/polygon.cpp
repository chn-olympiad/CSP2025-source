...
constexpr int N=5e3,Mod=998244353;
int n,a[N+5],g[N+5][N+5],ans;
void Mo(int &x,int y){x+=y;x>=Mod?x-=Mod:0;}
...
int main(){
	...
	for(int i=0;i<n;++i){
		for(int j=0;j<=N+1;++j){
			Mo(g[i+1][min(j+a[i+1],N+1)],g[i][j]);
			Mo(g[i+1][j],g[i][j]);
		}
	}
	for(int i=1;i<=n;++i){
		for(int j=N;j>=0;--j) Mo(g[i][j],g[i][j+1]);
	}
	for(int i=1;i<=n;++i) Mo(ans,g[i-1][a[i]+1]);
	fout<<ans<<"\n";
	return 0;
}
