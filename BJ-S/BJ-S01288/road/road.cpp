...
// Problem: 2025CSP-S2#T2. 道路修复road
// 1654.cpp Wrong Answer on Sample 1, Time Limit Exceeded on Sample 2, 3, 4
// 1714.cpp ?
#include <bits/stdc++.h>
using namespace std;
int n, m, k, c[15][10100], a[15];
vector<pair<int, int>> edges[10100];
priority_queue<pair<long long, pair<set<int>, set<int>>>, vector<pair<long long, pair<set<int>, set<int>>>>, greater<pair<long long, pair<set<int>, set<int>>>>> q;
inline void solve() {
	scanf ("%d%d%d", &n, &m, &k);
	for (int i = 1; i <= m; ++i) {
		int u, v, w;
	...
	q.push ({0, {set<int> (), set<int> ()}});
	while (!q.empty ()) {
		long long cost = q.top ().first;
		set<int> cities = q.top ().second.first, to_city = q.top ().second.second;
		q.pop ();
		if (cities.empty ()) {
			cities.insert (1);
			q.push ({cost, {cities, to_city}});
			continue;
		}
		#ifdef Happy_David_3367_FLAG /* 1714.cpp added */
			printf ("cost = %lld, cities = {", cost); /* 1714.cpp added */
			for (auto i : cities) { /* 1714.cpp added */
				printf ("%d, ", i); /* 1714.cpp added */
			} /* 1714.cpp added */
			printf ("\b\b}, to_city = {"); /* 1714.cpp added */
			for (auto i : to_city) { /* 1714.cpp added */
				printf ("%d, ", i); /* 1714.cpp added */
			} /* 1714.cpp added */
			printf ("\b\b}.\n"); /* 1714.cpp added */
		#endif /* 1714.cpp added */
		if (cities.size () == n) {
			printf ("%lld\n", cost);
			break;
		}
		for (auto u : cities) {
			for (auto i : edges[u]) {
				...
				if (cities.find (v) == cities.end ()) {
					set<int> cities2 = cities;
					cities2.insert (v);
					q.push ({cost + w, {cities2, to_city}});
				}
			}
		}
		for (int i = 1; i <= k; ++i) {
			if (to_city.find (i) == to_city.end ()) {
				set<int> to_city2 = to_city;
				to_city2.insert (i);
				q.push ({cost + a[i], {cities, to_city2}});
			}
		}
		for (auto u : to_city) {
			for (int i = 1; i <= n; ++i) {
				if (cities.find (i) == cities.end ()) {
					set<int> cities2 = cities;
					cities2.insert (i);
					q.push ({cost + c[u][i], {cities2, to_city}});
				}
			}
		}
...
int main() {
		...
		freopen ("1714.road1.out", "w", stdout);
		solve ();
		fclose (stdin);
		fclose (stdout);
		freopen ("road2.in", "r", stdin);
		freopen ("1714.road2.out", "w", stdout);
		solve ();
		fclose (stdin);
		fclose (stdout);
		freopen ("road3.in", "r", stdin);
		freopen ("1714.road3.out", "w", stdout);
		solve ();
		fclose (stdin);
		fclose (stdout);
		freopen ("road4.in", "r", stdin);
		freopen ("1714.road4.out", "w", stdout);
		solve ();
		fclose (stdin);
		fclose (stdout);
	#endif
