...
int fa[200005],len,a[15][200005],c[15];
bool vis[15];
ll ans=0,sum=0;
...
void dfs2(int x){
	...
	vis[x]=1;
	ll tmp=calc();
	if(tmp<=ans+3e8)cnk(ans,tmp),dfs2(x+1);
	vis[x]=0;
	tmp=calc();cnk(ans,tmp);
	dfs2(x+1);
}
int main(){
	...
	mst();ans=sum;
	int lim=1;for(int i=1;i<=K;i++)lim<<=1;
	lim=lim*(K*n+m)*std::__lg(K*n+m);
	if(lim<=400000000)dfs(1);
	else dfs2(1);
	printf("%lld\n",ans);
	return 0;
}
