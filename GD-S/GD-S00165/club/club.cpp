...
void solve(){
	...
	int sm=0;
	for (int i=1;i<=n;++i){
		if (a[i][0]>=a[i][1]&&a[i][0]>=a[i][2])b[i]=0,c[0]++,sm+=a[i][0];
		else if (a[i][1]>=a[i][0]&&a[i][1]>=a[i][2])b[i]=1,c[1]++,sm+=a[i][1];
		else b[i]=2,c[2]++,sm+=a[i][2];
	}
	int t=0;
	for (int i=0;i<3;i++)if (c[i]>n/2)t=i;
	vector<int> ve;
	for (int i=1;i<=n;++i){
		if (b[i]==t)ve.push_back(min(llabs(a[i][t]-a[i][(t+1)%3]),llabs(a[i][t]-a[i][(t+2)%3])));
	}
//	for (auto&i:ve)cout<<i<<' ';
//	cout<<'\n';
	sort(ve.begin(),ve.end());
	for (int i=0;i<c[t]-(n/2);++i)sm-=ve[i];
	cout<<sm<<'\n';
}
signed main(){
	ios::sync_with_stdio(0),cin.tie(nullptr),cout.tie(nullptr);
//	ios::sync+with+stdio(0),cin.tie(nullptr),cout.tie(nullptr);
//	freopen("D:\\club\\club5.in","r",stdin);
